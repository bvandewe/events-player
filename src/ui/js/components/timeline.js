...
import { Chart, CategoryScale, LinearScale, TimeScale, BarController, BarElement, LineElement, PointElement, Title, Tooltip, Legend } from 'chart.js';
...
Chart.register(CategoryScale, LinearScale, TimeScale, BarController, BarElement, LineElement, PointElement, Title, Tooltip, Legend);
...
class TimelineController {
            ...
            options: {
                ...
                onClick: async (event, elements) => {
                    ...
                    if (elements.length > 0) {
                        ...
                        console.log('[Timeline] Bar clicked, filtering by time range:', format(new Date(startTime), 'PPpp'), 'to', format(new Date(endTime), 'PPpp'));
    ...
    scheduleRefresh(immediate = false) {
        ...
        const refreshInterval = this.autoRefreshEnabled ? Math.max(this.getBucketSizeMs(), 500) : 2000;
    ...
    async refreshChart() {
        ...
        try {
            ...
            if (bucketTimes.length > 0) {
                ...
                this.lastBucketEndTime = finalBucketTime !== null ? finalBucketTime + bucketSizeMs : null;
            } else {
                this.lastBucketStartTime = null;
                this.lastBucketEndTime = null;
            }
    ...
    updateStats(buckets, bucketSpanCount = 0, firstBucketTime = null, lastBucketTime = null, bucketSizeMs = this.getBucketSizeMs()) {
        ...
        if (this.statElements.totalEventsTime) {
            this.statElements.totalEventsTime.textContent = timeSpanMs > 0 ? formatDistanceToNow(new Date(firstTime), { addSuffix: false }) : 'Now';
        }
