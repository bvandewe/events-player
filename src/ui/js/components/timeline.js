...
class TimelineController {
  constructor(storageManager) {
    ...
    this.currentZoomIndex =
      savedZoomIndex !== null ? parseInt(savedZoomIndex, 10) : 12;

    // Validate loaded index
    if (
      this.currentZoomIndex < 0 ||
      this.currentZoomIndex >= this.zoomLevels.length
    ) {
    ...
    this.autoRefreshEnabled =
      savedAutoRefresh === null ? true : savedAutoRefresh === "true";
  ...
  async init() {
    ...
    this.autoRefreshContainer =
      this.autoRefreshToggle?.closest(".timeline-auto-refresh-indicator") ||
      null;
    this.statElements = {
    ...
    if (
      this.chartScrollArea &&
      this.chartScrollArea.dataset &&
      this.chartScrollArea.dataset.visibleBuckets
    ) {
      const parsedBuckets = parseInt(
        this.chartScrollArea.dataset.visibleBuckets,
        10,
      );
    ...
    this.timelineTabTrigger = document.querySelector(
      '[data-bs-target="#timeline-pane"]',
    );
    if (this.timelineTabTrigger) {
      ...
      this.timelineTabTrigger.addEventListener(
        "shown.bs.tab",
        this.timelineTabHandler,
      );
    }
  ...
  initChart() {
    ...
    if (!canvasElement) {
      console.warn(
        '[Timeline] Canvas element "timelineChart" not found in DOM',
      );
      return;
    }
      ...
      options: {
        ...
        plugins: {
          ...
          legend: {
            ...
            onClick: (e, legendItem, legend) => {
              ...
              console.log(
                "[Timeline] Legend clicked, filtering by source:",
                source,
              );
        ...
        onClick: async (event, elements) => {
          ...
          if (elements.length > 0) {
            ...
            console.log(
              "[Timeline] Bar clicked, filtering by time range:",
              format(new Date(startTime), "PPpp"),
              "to",
              format(new Date(endTime), "PPpp"),
            );
            console.log("[Timeline] Raw timestamps:", {
              startTime,
              endTime,
              bucketTime,
              bucketSizeMs,
            ...
            if (this.currentZoomIndex > 0) {
              ...
              localStorage.setItem(
                "timeline_bucket_size",
                this.currentZoomIndex,
              );
              console.log(
                "[Timeline] Zooming in to bucket size:",
                this.getBucketSize(),
              );
  ...
  scheduleRefresh(immediate = false) {
    ...
    if (!this.autoRefreshEnabled && !immediate) {
      console.log(
        "[Timeline] Auto-refresh disabled, skipping scheduled refresh",
      );
      return;
    }
  ...
  async refreshChart() {
    if (!this.storageManager || !this.chart) {
      console.warn(
        "[Timeline] Cannot refresh chart - storage or chart not initialized",
      );
      return;
    }
    ...
    try {
      ...
      const events = await this.storageManager.getRecentEvents({
        ...filterOptions,
        limit: 100000,
      });
      ...
      if (!events || events.length === 0) {
        ...
        try {
          ...
          console.error(
            "[Timeline] Chart update error (no events):",
            chartError,
          );
        }
      ...
      events.forEach((event) => {
        ...
        if (
          !eventTimeStr.endsWith("Z") &&
          !eventTimeStr.includes("+") &&
          !eventTimeStr.includes("T00:00:00")
        ) {
        ...
        if (Number.isNaN(timestamp)) {
          console.warn(
            "[Timeline] Skipping event with invalid timestamp",
            event,
          );
          return;
        }
      ...
      if (
        minEventTimestamp !== Number.POSITIVE_INFINITY &&
        maxEventTimestamp !== Number.NEGATIVE_INFINITY
      ) {
        this.autoFitMinTimestamp = minEventTimestamp;
        const nowTimestamp = Date.now();
        this.autoFitMaxTimestamp = this.autoRefreshEnabled
          ? Math.max(maxEventTimestamp, nowTimestamp)
          : maxEventTimestamp;
        this.timelineRangeMs = Math.max(
          this.autoFitMaxTimestamp - this.autoFitMinTimestamp,
          0,
        );
      } else {
        this.autoFitMinTimestamp = null;
      ...
      if (bucketTimes.length > 0) {
        ...
        const shouldFillMissingBuckets =
          expectedBucketCount > bucketTimes.length &&
          expectedBucketCount <= MAX_BUCKET_FILL_COUNT;
        ...
        this.lastBucketEndTime =
          finalBucketTime !== null ? finalBucketTime + bucketSizeMs : null;
      } else {
        this.lastBucketStartTime = null;
        this.lastBucketEndTime = null;
      }
      ...
      if (renderBucketTimes.length > 0 && this.chart.options?.scales?.x) {
        ...
        const lastRenderBucket =
          renderBucketTimes[renderBucketTimes.length - 1];
        xScale.max =
          lastRenderBucket !== undefined
            ? lastRenderBucket + bucketSizeMs
            : undefined;
      }
      ...
      try {
        ...
        if (
          chartError.message &&
          chartError.message.includes("too far apart with stepSize")
        ) {
          // Extract timestamps from error message if possible
          const match = chartError.message.match(
            /(\d+)\s+and\s+(\d+)\s+are too far apart with stepSize of (\d+)\s+(\w+)/,
          );
          ...
          actionsController.showError({
            ...
            message:
              "The selected bucket size is too small for the time range of your events. Please try using a larger bucket size (e.g., minutes or hours instead of seconds).",
      ...
      requestAnimationFrame(() =>
        this.adjustChartViewport(effectiveBucketSpan, wasPinnedToEnd),
      );
      ...
      const sortedBuckets = renderBucketTimes.map((time) => {
        ...
        const count = Object.values(buckets[time]).reduce(
          (sum, c) => sum + c,
          0,
        );
        return { time, count };
      ...
      console.log(
        "[Timeline] Chart refreshed with",
        events.length,
        "events across",
        renderBucketTimes.length,
        "rendered buckets (span:",
        effectiveBucketSpan,
        ") and",
        sourceArray.length,
        "sources",
  ...
  updateStats(
    ...
    lastBucketTime = null,
    bucketSizeMs = this.getBucketSizeMs(),
  ) {
    ...
    // Time span
    const firstTime =
      firstBucketTime !== null ? firstBucketTime : buckets[0].time;
    const lastTime =
      lastBucketTime !== null
        ? lastBucketTime
        : buckets[buckets.length - 1].time;
    const timeSpanMs = Math.max(
      lastTime - firstTime + bucketSizeMs,
      bucketSizeMs,
    );
    ...
    if (this.statElements.totalEventsTime) {
      this.statElements.totalEventsTime.textContent =
        timeSpanMs > 0
          ? formatDistanceToNow(new Date(firstTime), { addSuffix: false })
          : "Now";
    }
    ...
    if (this.statElements.peakRate) {
      this.statElements.peakRate.textContent =
        peakBucket.count.toLocaleString();
    }
    if (this.statElements.peakRateTime) {
      this.statElements.peakRateTime.textContent = format(
        new Date(peakBucket.time),
        "PPpp",
      );
    }
    ...
    if (this.statElements.quietPeriodsSize) {
      const bucketInfo = this.getBucketSize();
      this.statElements.quietPeriodsSize.textContent = `${bucketInfo.value} ${bucketInfo.unit}${bucketInfo.value > 1 ? "s" : ""}`;
    }
  }
  ...
  adjustChartViewport(
    bucketCount = this.rawBucketTimes.length,
    maintainEndPosition = false,
  ) {
    ...
    const desiredWidth = Math.max(
      safeAvailableWidth,
      Math.ceil(effectiveBucketCount * bucketWidth),
    );
  ...
  async setBucketSize(newIndex) {
    ...
    if (currentFilters.timeRange && currentFilters.timeRange !== "all") {
      console.log(
        "[Timeline] Resetting time range filter due to bucket size change",
      );
      appState.set("filters", {
        ...currentFilters,
        timeRange: "all",
  ...
  async handleAutoFit() {
    ...
    this.zoomLevels.forEach((level, index) => {
      const bucketSizeMs =
        level.unit === "second" ? level.value * 1000 : level.value * 60 * 1000;
  ...
  updateAutoRefreshVisualState() {
    ...
    if (this.autoRefreshContainer) {
      this.autoRefreshContainer.classList.toggle(
        "highlighted",
        !this.autoRefreshEnabled,
      );
    }
  ...
  destroy() {
    ...
    if (this.timelineTabTrigger && this.timelineTabHandler) {
      this.timelineTabTrigger.removeEventListener(
        "shown.bs.tab",
        this.timelineTabHandler,
      );
    }
