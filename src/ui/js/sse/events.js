...
export const sseEventsController = (() => {
  ...
  const createAccordionItem = ({
    ...
    filterButtonsContainer.classList.add(
      "d-flex",
      "gap-1",
      "ms-2",
      "align-middle",
    ...
    const sourceButton = createFilterButton(
      "source",
      eventSource,
      "geo-alt",
      `Filter by source: ${eventSource}`,
    );
    const typeButton = createFilterButton(
      "type",
      eventType,
      "funnel",
      `Filter by type: ${eventType}`,
    );
    const subjectButton = createFilterButton(
      "subject",
      eventSubject || "",
      "tag",
      `Filter by subject: ${eventSubject || "(none)"}`,
    );
  ...
  const updateFilteredCount = () => {
    ...
    if (hasActiveFilters) {
      ...
      if (eventsStack) {
        const visibleEvents = eventsStack.querySelectorAll(
          ".accordion-item:not(.d-none)",
        ).length;
        appState.setFilteredEventCount(visibleEvents);
        console.log(
          "[Events] Updated filtered count:",
          visibleEvents,
          "/",
          totalCount,
        );
    ...
    if ("data" in event) {
      ...
      try {
        ...
        if (!isValidCloudEvent) {
          console.info(
            "[Events] Ignoring non-CloudEvent payload from SSE stream",
            { payload: cloudEventData },
          );
          return;
        }

        const isHandshakeEvent =
          cloudEventData &&
          typeof cloudEventData.system === "string" &&
          (!cloudEventData.type || cloudEventData.type === "system.connected");

        if (isHandshakeEvent) {
          console.info(
            "[Events] Ignoring handshake/system event from SSE stream",
            { payload: cloudEventData },
          );
          return;
        }
        ...
        if (typeof cloudEventData.data === "object") {
          if (
            Object.keys(cloudEventData.data).length == 1 &&
            cloudEventData.data.hasOwnProperty("error")
          ) {
        ...
        var strippedEventData =
          eventData.substring(0, eventData.indexOf(', "data"')) + "}}";
      ...
      if (!cloudEventData || typeof cloudEventData !== "object") {
        console.warn(
          "[Events] No CloudEvent data found after parsing SSE payload, skipping event",
        );
        return;
      }
      ...
      if (!hasRequiredFields) {
        console.warn(
          "[Events] CloudEvent missing required attributes after parse, skipping event",
          cloudEventData,
        );
        return;
      }
      ...
      if (isSystemEvent) {
        console.info(
          "[Events] Ignoring system Cloudevent after parse validation",
          cloudEventData,
        );
        return;
      }
      ...
      const timestampEl = item.querySelector(
        '.event-timestamp[data-bs-toggle="tooltip"]',
      );
      if (timestampEl) {
  ...
  const loadEventsFromStorage = async () => {
    try {
      ...
      const startTime = urlParams.get("startTime")
        ? parseInt(urlParams.get("startTime"))
        : null;
      const endTime = urlParams.get("endTime")
        ? parseInt(urlParams.get("endTime"))
        : null;
      const typeFilter = urlParams.get("type") || null;
      const sourceFilter = urlParams.get("source") || null;
      const subjectFilter = urlParams.has("subject")
      ...
      if (!startTime && !endTime) {
        ...
        if (timeRange && timeRange !== "all") {
          ...
          if (timeRange === "custom") {
            ...
            console.log(
              "[Events] Applying custom time range from global filters:",
              {
                start: timeRangeStart
                  ? new Date(timeRangeStart).toISOString()
                  : "none",
                end: timeRangeEnd
                  ? new Date(timeRangeEnd).toISOString()
                  : "none",
              },
            );
          } else {
      ...
      console.log(
        "[Events] getRecentEvents returned:",
        events ? events.length : "null/undefined",
        "events",
      );
      ...
      if (events && hasActiveFilters) {
        ...
        events = events.filter((event) => {
          ...
          if (timeRangeStart || timeRangeEnd) {
            ...
            if (
              eventTime &&
              !eventTime.endsWith("Z") &&
              !eventTime.includes("+") &&
              !eventTime.includes("-", 10)
            ) {
            ...
            console.log("[Events] Checking event time:", {
              ...
              endTime: timeRangeEnd,
              inRange:
                (!timeRangeStart || eventTimestamp >= timeRangeStart) &&
                (!timeRangeEnd || eventTimestamp < timeRangeEnd),
            });
          ...
          if (activeSourceFilter && event.source !== activeSourceFilter) {
            console.log(
              "[Events] Filtering out by source:",
              event.source,
              "!==",
              activeSourceFilter,
            );
          ...
          const activeSubjectFilter =
            subjectFilter !== null ? subjectFilter : stateFilters.subject;
          if (
            activeSubjectFilter !== null &&
            event.subject !== activeSubjectFilter
          ) {
            console.log(
              "[Events] Filtering out by subject:",
              event.subject,
              "!==",
              activeSubjectFilter,
            );
      ...
      if (events && events.length > 0) {
        ...
        if (!eventsStack) {
          console.log(
            "[Events] Skipping event display - not on events list page",
          );
          return;
        }
        ...
        console.log(
          `[Events] Displayed ${events.length} events out of ${totalEventsCount} total`,
        );
        ...
        console.log(
          "[Events] No events found in storage (events array empty or null)",
        );
  ...
  const init = (queueSize, storageManager) => {
    ...
    document.addEventListener("visibilitychange", () => {
      if (!document.hidden && eventStorageManager && eventStorageManager.db) {
        console.log(
          "[Events] Page became visible, reloading events from storage...",
        );
        loadEventsFromStorage();
      }
    });
    ...
    const setupTimestampToggle = () => {
      ...
      if (timestampToggle) {
        timestampToggle.addEventListener("click", () => {
          ...
          const newFormat =
            currentFormat === "relative" ? "absolute" : "relative";

          console.log(
            `[Events] Toggling timestamps from ${currentFormat} to ${newFormat}`,
          );
