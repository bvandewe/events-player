export const searchController = (() => {
  ...
  const applySearch = (searchTerm) => {
    ...
    const eventMessages =
      eventsStackDiv.getElementsByClassName("accordion-item");
    let visibleCount = 0;
    let hiddenCount = 0;
    ...
    console.log(
      `[Search] Applied search "${trimmedTerm}": ${visibleCount} visible, ${hiddenCount} hidden`,
    );
  ...
  const updateSearchActiveState = (searchTerm) => {
    ...
    if (filterInput) {
      ...
      filterInput.setAttribute(
        "aria-label",
        isActive
          ? `Search events (filtering for "${trimmedTerm}")`
          : DEFAULT_ARIA_LABEL,
      );
    }
    ...
    if (badge) {
      if (isActive) {
        const displayTerm =
          trimmedTerm.length > 24
            ? `${trimmedTerm.slice(0, 21)}...`
            : trimmedTerm;
