...
class EventStorageManager {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onupgradeneeded = (event) => {
        ...
        if (!db.objectStoreNames.contains("recentEvents")) {
          const recentStore = db.createObjectStore("recentEvents", {
            keyPath: "id",
          });
          recentStore.createIndex("timestamp", "timestamp", { unique: false });
          recentStore.createIndex("insertionOrder", "insertionOrder", {
            unique: true,
          });
  ...
  async addEvent(fullEvent) {
    ...
    try {
      if (!fullEvent || !fullEvent.id) {
        console.warn(
          "[EventStorage] Ignoring event without id field",
          fullEvent,
        );
        return false;
      }
      ...
      try {
        ...
        console.warn(
          "[EventStorage] Metadata extraction threw error, skipping event",
          {
            error: metadataError,
            event: fullEvent,
          },
      ...
      if (!metadata) {
        console.warn(
          "[EventStorage] Skipping event due to invalid metadata",
          fullEvent,
        );
        return false;
      }
      ...
      console.log(
        `[EventStorage] Storing full event ${fullEvent.id} in recentEvents store`,
      );
  ...
  async addRecentEvent(event) {
    return new Promise((resolve, reject) => {
      ...
      request.onerror = () => {
        console.error(
          "[EventStorage] Error adding recent event:",
          request.error,
        );
        reject(request.error);
      };
  ...
  extractMetadata(event) {
    ...
    if (!event.time) {
      console.warn(
        "[EventStorage] Event missing time field, cannot extract metadata",
        event,
      );
      return null;
    }
    ...
    if (Number.isNaN(timestamp)) {
      console.warn("[EventStorage] Unable to parse event timestamp", {
        eventTime,
        event,
      });
      return null;
    ...
    return new Promise((resolve, reject) => {
      ...
      countRequest.onsuccess = async () => {
        ...
        if (totalCount > this.maxRecentEvents) {
          ...
          console.log(
            `[EventStorage] Over capacity (${totalCount}/${this.maxRecentEvents}), removing ${excess} oldest events`,
          );
      ...
      countRequest.onerror = () => {
        console.error(
          "[EventStorage] Error counting events for cleanup:",
          countRequest.error,
        );
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = (event) => {
        ...
        if (cursor && deleted < count) {
          ...
          if (deleted > 0) {
            console.log(
              `[EventStorage] Deleted ${deleted} oldest events (over limit)`,
            );
          }
          resolve(deleted);
        }
      ...
      request.onerror = () => {
        console.error(
          "[EventStorage] Error deleting oldest events:",
          request.error,
        );
        reject(request.error);
      };
    ...
    return new Promise((resolve, reject) => {
      ...
      countRequest.onsuccess = async () => {
        ...
        if (totalCount > this.maxMetadataEvents) {
          ...
          console.log(
            `[EventStorage] Metadata over capacity (${totalCount}/${this.maxMetadataEvents}), removing ${excess} oldest entries`,
          );
      ...
      countRequest.onerror = () => {
        console.error(
          "[EventStorage] Error counting metadata for cleanup:",
          countRequest.error,
        );
        reject(countRequest.error);
      };
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = (event) => {
        ...
        if (cursor && deleted < count) {
          ...
          if (deleted > 0) {
            console.log(
              `[EventStorage] Deleted ${deleted} oldest metadata entries (over limit)`,
            );
          }
          resolve(deleted);
        }
      ...
      request.onerror = () => {
        console.error(
          "[EventStorage] Error deleting oldest metadata:",
          request.error,
        );
        reject(request.error);
      };
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = (event) => {
        ...
        if (cursor && events.length < limit) {
          ...
          if (filter.startTime && evt.timestamp < filter.startTime)
            include = false;
      ...
      request.onerror = () => {
        console.error(
          "[EventStorage] Error querying recent events:",
          request.error,
        );
        reject(request.error);
      };
  ...
  async getEventById(id) {
    return new Promise((resolve, reject) => {
      ...
      request.onerror = () => {
        console.error(
          "[EventStorage] Error getting event by ID:",
          request.error,
        );
        reject(request.error);
      };
  ...
  async getAggregatedStats(
    ...
    metadata.forEach((m) => {
      ...
      if (m.subject) {
        bucket.subjects.set(
          m.subject,
          (bucket.subjects.get(m.subject) || 0) + 1,
        );
      }
  ...
  async getUniqueValues(field) {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onerror = () => {
        console.error(
          "[EventStorage] Error getting unique values:",
          request.error,
        );
        reject(request.error);
      };
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        if (persistedStats) {
          ...
          this.stats.lastCleanup = parsed.lastCleanup
            ? new Date(parsed.lastCleanup)
            : null;
          console.log(
            "[EventStorage] Loaded persisted stats from localStorage:",
            {
        ...
        console.warn(
          "[EventStorage] Failed to load persisted stats from localStorage:",
          err,
        );
      }
        ...
        if (countsLoaded === 2) {
          ...
          oldestRequest.onsuccess = (e) => {
            ...
            newestRequest.onerror = () => {
              console.error(
                "[EventStorage] Error getting newest metadata:",
                newestRequest.error,
              );
          ...
          oldestRequest.onerror = () => {
            console.error(
              "[EventStorage] Error getting oldest metadata:",
              oldestRequest.error,
            );
      ...
      metadataCountRequest.onerror = () => {
        console.error(
          "[EventStorage] Error loading metadata stats:",
          metadataCountRequest.error,
        );
      ...
      recentCountRequest.onerror = () => {
        console.error(
          "[EventStorage] Error counting recent events:",
          recentCountRequest.error,
        );
  ...
  persistStats() {
    try {
      ...
      console.warn(
        "[EventStorage] Failed to persist stats to localStorage:",
        err,
      );
    }
  }
    ...
    return new Promise((resolve, reject) => {
      ...
      const checkComplete = () => {
        if (recentEventsCleared && metadataCleared) {
          ...
          this.persistStats();
          console.log(
            "[EventStorage] All data cleared (recentEvents and metadata)",
          );
          resolve();
        }
      };
      ...
      clearRecentEvents.onerror = () => {
        console.error(
          "[EventStorage] Error clearing recent events:",
          clearRecentEvents.error,
        );
      ...
      clearMetadata.onerror = () => {
        console.error(
          "[EventStorage] Error clearing metadata:",
          clearMetadata.error,
        );
